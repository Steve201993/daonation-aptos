...
export class CommunityService {
  ...
  static async getByapsosReferenceId(apsosReferenceId: string): Promise<ApiCommunity> {
    const response = await fetch(`/api/communities/${apsosReferenceId}`);
    if (!response.ok) {
  ...
  static async updateByapsosReferenceId(apsosReferenceId: string, updateData: Partial<ApiCommunity>): Promise<ApiCommunity> {
    const response = await fetch(`/api/communities/${apsosReferenceId}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json'
  ...
  static async deleteByapsosReferenceId(apsosReferenceId: string): Promise<void> {
    const response = await fetch(`/api/communities/${apsosReferenceId}`, {
      method: 'DELETE'
    });
